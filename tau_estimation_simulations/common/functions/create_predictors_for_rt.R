create_predictors_for_rt = function(rt) {
  ...
  if (var(rt) > 0) {
    ...
    tryCatch({
      model = gamlss(rt ~ 1, family = exGAUS(), data = data.frame(rt))
  ...
  X$lasso_factor   = as.numeric(predict(lasso_model, t(as.numeric(X)), s = "lambda.min"))
  X$rf_factor      = as.numeric(predict(rf_model_cv, X))
  X$exGaus_fit     = as.numeric(nu_coef)
