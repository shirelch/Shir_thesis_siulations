...
prior_density             = dunif(possible_threshold_values, config$min_coherence, config$max_coherence)
...
for (guessed_true_slope in guessed_true_slope_values) {
  for (i in seq_len(nrow(agents))) {
    ...
    for (trial in 1:config$Ntrials) {
      ...
      likelihood = config$prob_correct(coherence, lambda, config$guess_rate, possible_threshold_values, guessed_true_slope)
      ...
      # Append data
      df = rbind(df, data.frame(trial, acc = acc, coherence = coherence,
                                t = t, beta = beta))
  ...
  save(all_results, file = paste0("./adaptive_optimization_simulations/data/simulation_results/quest_beta_", guessed_true_slope, "_simulation_results.RData"))
}
