...
prior_density = outer(rep(
  1 / length(possible_threshold_values),
  length(possible_threshold_values)
),
rep(
  1 / length(possible_slope_values),
  length(possible_slope_values)
))
...
simulate_quest_plus_parallel =
  function(agents_df, config, out_file) {
    ...
    results_list = future_lapply(agents_list, function(agent_trials) {
      ...
      for (trial in seq_len(nrow(agent_trials))) {
        ...
        # Compute likelihood only at the presented stimulus over the parameter grid
        likelihood_matrix = compute_likelihood(coherence,
                                               lambda,
                                               config$guess_rate,
                                               param_grid_t,
                                               param_grid_beta)
        ...
        # Candidate selection: For each candidate stimulus, compute expected entropy
        expected_entropy = sapply(candidate_coherences, function(stim) {
          # Compute likelihood at candidate stimulus:
          likelihood_stim = compute_likelihood(stim,
                                               lambda,
                                               config$guess_rate,
                                               param_grid_t,
                                               param_grid_beta)
