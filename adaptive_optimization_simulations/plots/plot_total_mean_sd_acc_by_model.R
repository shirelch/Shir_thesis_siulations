...
total_sd_acc = data.frame(Model = character(), MeanAccuracy = numeric(), stringsAsFactors = FALSE)
...
for (file_path in file_paths) {
  ...
  if (exists("all_results")) {
    ...
    sd_acc = sd(all_results$acc, na.rm = TRUE)
    ...
    total_sd_acc = rbind(
      total_sd_acc,
      data.frame(Model = dataset_name, SDAccuracy = sd_acc)
    ) 
...
p = ggplot(total_sd_acc, aes(x = Model, y = SDAccuracy, fill = Model)) +
  geom_bar(stat = "identity") +
  geom_text(aes(label = round(SDAccuracy, 3)), 
            vjust = -0.3,
            size = 3) +
  labs(
    title = "Total SD Accuracy Per Model",
    x = "Model",
    y = "Total SD Accuracy"
  ) +
  theme_minimal() +
